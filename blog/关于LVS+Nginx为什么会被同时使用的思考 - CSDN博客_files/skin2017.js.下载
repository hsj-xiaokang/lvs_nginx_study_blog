...
$(function(){
    ...
    var oBlog = {
        ...
        readMore:function(){
            ...
            function bindRead_more(){
                if(!article_show){
                    $('.article_content').height(widHeight*2).css({'overflow':'hidden'});
        ...
        leftMenuShow:function(){
            ...
            var FixedLeftMenu = $('main').offset().left-50;
            if (winWdith>1280) {
              $('.left_fixed').css({'left':FixedLeftMenu});
            }else{
              $(".left_show_button").css({display:'block'}).animate({left:'+40'},300)
              left_fixed_event()
              $('.left_fixed')
              $('.left_fixed').css({'left':-40}).children('ul').addClass('by_left');
            }
        ...
        likeFn:function(){
            var likeBtn = $(".btn-like"),
                ...
                $btnNum = $('.left_fixed .btn-like .txt');
            if (currentUserName) {
                $.get(blog_address + "/article/IsDigg?ArticleId=" + fileName, function (data) {
                    if(data.status==0){
                        ...
                        $('.btn-like i').removeClass('icon-dianzan').addClass('icon-yidianzan');
                    }
                ...
                if (currentUserName) {
                    $.get(blog_address + "/article/digg?ArticleId=" + fileName, function (data) {
                        if(data.status==0){
                          ...
                          $btnNum.text(parseInt($btnNum.text())-1);
                          BlogData.liked = false;
                          $('.btn-like i').removeClass('icon-yidianzan').addClass('icon-dianzan');
                        }else{
        ...
        hotcolumnlist:function(){
          var $columnListDOM = $('.column-list'),
              columnListDOMOldHeight = $columnListDOM.height()+20 ,
        ...
        rightFixedRightSide:function(){
          ...
          if(windowW > 1440){
            $right_fixed.css({'right': '3%'})
          }else {
            $right_fixed.css({'right': '1%'})
          }
        },
        isAdm:function(){
          try {
            if(!!currentUserName && isAdm){
              $('.right_bar .del').css({'display':'block'});
            }
          } catch (e) {
          }
        }
    ...
    oBlog.deleteTagline();
    oBlog.hotcolumnlist();
    oBlog.rightFixedRightSide(); // 初始化右边 .right_fixed
    oBlog.isAdm();
...
// 外部blog方法，多是监听事件
var outoBlog = {
  left_menu_isHide:false, // 左侧按钮是否隐藏
  leftMenuAutoMove:function(){  // 左侧菜单位置自动调整
    var WindowWidth = $(window).width(),
        $left_show_button = $('.left_show_button'),
    ...
    if(WindowWidth < 1280){
      ...
      $left_fixed.css({'left': -40}).children('ul').addClass('by_left');
      $left_show_button.css({display:'block'}).animate({left:'+40'},300)
      this.left_menu_isHide = true;
      left_fixed_event()
    }else {
      $left_fixed.css({'left':FixedLeftMenu});
      if(!this.left_menu_isHide) return ;
      $left_fixed.children('ul').removeClass('by_left');
      $left_show_button.css({display:'none'}).animate({left:'0'},300)
      this.left_menu_isHide = false;
      $('.left_fixed').unbind('mouseenter').unbind('mouseleave');
    }
  },
  ...
  bottomRcomAutoPosition:function(){
    var $bottomRcom = $('.bottomRcom'),
        $article = $('article'),
        articleOffsetLeft = $article.offset().left,
        articleW = $article.width();
    $bottomRcom.css({left:articleOffsetLeft+articleW+20})
  }
}
// 监听事件整合
var oBlogResize =function(){
    ...
    outoBlog.rightFixedRightSide();
    outoBlog.bottomRcomAutoPosition();
}
...
// 左侧hover 更换 icon 
// 点赞
$('.left_fixed .btn-like').hover(function(){
  $('.left_fixed .btn-like i').removeClass('icon-dianzan').addClass('icon-yidianzan')
},function(){
  if(BlogData.liked) return false;
  $('.left_fixed .btn-like i').removeClass('icon-yidianzan').addClass('icon-dianzan')
})
// 评论
...
// 分享
$('#share_box button.left-fixed-btn').hover(function(){
  $('#share_box button.left-fixed-btn i').removeClass('icon-fenxiang').addClass('icon-fenxianghover')
},function() {
  $('#share_box button.left-fixed-btn i').removeClass('icon-fenxianghover').addClass('icon-fenxiang')
...
function left_fixed_event(){
  ...
  $('.left_fixed').hover(function(){
    $('.left_fixed').stop(true, false)
    if($('.left_fixed').is(':animated')){
      $('.left_fixed').css({left:0})
    }else{
      $('.left_fixed').animate({left:0},500,function(){
        $('.left_show_button').css({display: 'none'})
      })
    }
  },function(){
   // function hideLeft(){
      $('.left_fixed').delay(1000).animate({left:-40},500,function(){
        $('.left_show_button').css({display: 'block'})
      })
    // }
    // // if(!$('.left_fixed').is(':animated')){
    //   setTimeout(hideLeft,1000)
    // // }
  })
};
$(function(){
  ...
  window.onload = adhoc('getFlags', applyFlags);
  function applyFlags(flags) {
    abFlags = flags.get('isLogin');
    if(!abFlags && !currentUserName){
      var isbindB = isReadButton();
      adhoc('track', 'login', 1);
      if(isbindB) bindBUtton();
    }
   }
   function isReadButton(){
     var isBlock = false;
     if($('.readall_box').css('display')=== 'block'){
       adhoc('track', 'readButtonshow', 1);
       isBlock = true
     };
     return isBlock;
   };
   function bindBUtton(){
     $('.read_more_btn').unbind('click').bind('click',function(){read_more_loginto(0)})
     $(document).on('click','.login_bt',function(){adhoc('track', 'loggingin', 1);})
     $(document).on('click','.pw_a',function(){adhoc('track', 'register', 1);})
     $(document).on('click','.lg_3 .qq',function(){adhoc('track', 'qq', 1);})
     $(document).on('click','.lg_3 .wechat',function(){adhoc('track', 'wechat', 1);})
     $(document).on('click','.csdn_loginbox .close',function(){adhoc('track', 'clickClose', 1);})
  
   }
   function read_more_loginto(n) {
       if (!n) {
           csdn.showLogin(function (data) {
               read_more_js_logined(data.data.userName);
           });
       } else {
           if (!getUN()) {
               csdn.showLogin(function (data) {
   ...
   function read_more_js_logined(un) {
       location = '?readlog';
   }
   (function(){
     if(window.location.search.indexOf('readlog')>-1 && !!currentUserName){
       adhoc('track', 'logIn_read', 1);
      $('.readall_box').hide().addClass('readall_box_nobg');
      $('.article_content').height("").css({'overflow':'hidden'});
     } 
   })()
